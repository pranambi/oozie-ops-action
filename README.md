...
| `Restart_failed_node` | Rerun only the failed node in a job | No |

---

## Demo — Sample job IDs for testing

The runner image includes a sample jobs list at `/etc/oozie/oozie-jobs.json`. Use these job IDs when triggering workflows in demo mode (no real Oozie server needed).
